...
## Visual Enhancement with Scientific Schematics (Optional)

This skill is fully usable for citation management on its own; nothing below is required for the core search, extraction, validation, and formatting workflows.

As an **optional** enhancement, you may add scientific diagrams when a document benefits from visual communication of a concept, workflow, or architecture.

There are two independent ways to do this, both optional:

- **External `scientific-schematics` skill** (not bundled in this repository): if you have it installed separately, it can generate AI diagrams from a natural-language description. It is not included here and is not a dependency of this skill.
- **Bundled helper scripts** (`scripts/generate_schematic.py`, `scripts/generate_schematic_ai.py`): these call the **OpenRouter** image API.

> **Paid API and network egress (important):** the bundled schematic scripts require a **paid** `OPENROUTER_API_KEY` and send your prompt over the network to OpenRouter (`https://openrouter.ai`). They are entirely optional and are **not** part of the citation happy-path. Skip this section if you do not want to use a paid third-party image service. See the Dependencies section for details.

**How to generate schematics (optional):**
```bash
# Requires a paid OPENROUTER_API_KEY and network access
...
- Review and refine through a small number of iterations
- Aim for accessibility (colorblind-friendly, high contrast)
- Save outputs to the path you pass with `-o`

**When schematics may help:**
...
If you have the external `scientific-schematics` skill installed, refer to its own documentation for detailed guidance.
...
# Extract from URL (supported: doi.org/dx.doi.org links, PubMed and arXiv URLs,
# or any URL that contains a literal DOI such as .../10.1371/journal.pone.0123456).
# Publisher landing pages whose slug is not the DOI (e.g. nature.com/articles/<slug>)
# are not resolved; use the DOI directly in that case.
python scripts/extract_metadata.py --url "https://doi.org/10.1038/s41586-021-03819-2"
...
# Validate and report issues (use the dedicated validator; it writes a JSON report)
...
# Fix common issues and write a cleaned file (the formatter applies the safe
# auto-fixes: page ranges, "pp." removal, author separators, field order)
python scripts/format_bibtex.py references.bib \
  -o validated_references.bib

# Generate detailed validation report (JSON)
python scripts/validate_citations.py references.bib \
  --report validation_report.json \
  --verbose
```
...
```bash
# 1. Search for papers on your topic and export BibTeX directly.
#    search_pubmed.py already fetches full metadata, so it can emit BibTeX
#    in one step (no separate extract_metadata.py pass is needed).
python scripts/search_pubmed.py \
...
# (Alternative) If you kept JSON results, pull identifiers out first.
# extract_metadata.py expects one DOI/PMID/arXiv ID per line, NOT a results
# JSON file, so extract the PMIDs before piping them in:
#   python -c "import json,sys; [print(r['pmid']) for r in json.load(open('crispr_papers.json'))['results'] if r.get('pmid')]" > ids.txt
#   python scripts/extract_metadata.py --input ids.txt --output crispr_refs.bib

# 3. Add specific papers by DOI
python scripts/doi_to_bibtex.py 10.1038/nature12345 >> crispr_refs.bib
python scripts/doi_to_bibtex.py 10.1126/science.abcd1234 >> crispr_refs.bib

# 4. Format and clean the BibTeX file (applies safe auto-fixes by default)
...
# 5. Validate all citations and write a JSON report
...
# Normalize and clean the .bib file (the visual citation style itself is set
# by the LaTeX bibliography style / .bst, not by this formatter)
...
# From URL (doi.org/dx.doi.org, PubMed, arXiv, or a URL containing a literal DOI)
...
**Features**:
- DOI verification via doi.org and CrossRef (with `--check-dois`)
...
# Verify that DOIs resolve via doi.org/CrossRef (slower, network access)
python scripts/validate_citations.py references.bib \
  --check-dois
```

To apply safe auto-fixes and write a cleaned file, use the formatter:
```bash
...
**Features**:
- Standardize formatting and field order
- Sort entries (by key, year, author, or title)
- Remove duplicates (by DOI and citation key)
- Fix common errors (page ranges, "pp.", DOI prefixes, author separators)
...
# Complete cleanup (common-issue fixes are applied automatically unless --no-fix)
python scripts/format_bibtex.py references.bib \
  --deduplicate \
  --sort year \
  --descending \
...
```bash
# Step 1: Find key papers on your topic and export BibTeX directly.
# Both search scripts fetch full metadata, so --format bibtex produces
# ready-to-use entries in one step (no separate extraction pass needed).
...
# Step 5: Format, deduplicate, and apply safe auto-fixes (run by default)
...
# Step 6: Validate and write a JSON report
python scripts/validate_citations.py final_references.bib \
  --report validation.json \
  --verbose
...
# Step 3: Validate and review issues (Step 1 already applied the safe auto-fixes)
...
```bash
# Find highly cited papers on a topic (JSON keeps the citation counts so you
# can review and pick the top entries before exporting).
...
# Review alphafold_seminal.json and note the citation counts.

# Export the same search directly to BibTeX (search_google_scholar.py builds
# the entries itself; there is no separate extraction step for its results).
python scripts/search_google_scholar.py "AlphaFold protein structure" \
  --year-start 2020 \
  --year-end 2024 \
  --sort-by citations \
