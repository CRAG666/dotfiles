...
> **Note:** MarkItDown is already installed in this environment (with all optional
> features). **Do not attempt to install it** — no `pip install`, `git clone`, or
> system-package commands are needed. Use the `markitdown` CLI and the `markitdown`
...
## Supported Format Extras

All optional features are already available in this environment (equivalent to the
`markitdown[all]` install), so every format below works out of the box:
...
> All dependencies are pre-installed — do not run install commands when
> troubleshooting. If something fails, check the input file and arguments instead.

1. **Binary file errors**: Ensure files are opened in binary mode
   ```python
   with open("file.pdf", "rb") as f:  # Note the "rb"
       result = md.convert_stream(f, file_extension=".pdf")
   ```

2. **OCR/format-specific errors**: All optional dependencies (including the OCR
   stack) are already available. If conversion fails, verify the file is not
   corrupt and that the correct `file_extension` is passed.
