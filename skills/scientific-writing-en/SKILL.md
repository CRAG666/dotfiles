...
Do NOT activate for code comments, docstrings, commit messages, casual emails, marketing copy, fiction, or general non-academic writing. For blog posts or popular-science pieces, ask before invoking the academic register - these usually want lighter, more accessible prose.

## Goal

Replace generic "LLM-default" prose with established academic English constructions (Academic Phrasebank tradition), and lift drafts to Q1-journal structural and rhetorical standards. See `references/core-principles.md` for what "Q1 quality" means and the 11 non-negotiable rules (hedging, tense discipline, US spelling, ASCII-only punctuation, citation grammar, etc.) - read this file on first use in a session, before drafting anything.

## Workflow

1. **Identify the section or function** the user needs (see index below).
2. **Read `references/core-principles.md`** if not already read this session.
3. **Read ONLY the matching reference file(s)** for that section/function. Never load more than 2-3 reference files for a single request - each runs several hundred lines.
4. **Draft finished prose**, not a phrase menu. Pick phrases from the bank, adapt them to the user's actual content (replace X/Y/Z/"Author" placeholders), and combine/vary them so the result reads as prose, not a phrasebook collage.
5. **Self-check before delivering** using `references/qa-checklist.md`.
...
Structural and stylistic craft (cross-cutting):
