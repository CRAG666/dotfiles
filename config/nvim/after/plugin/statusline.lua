...
M.build_palette = function()
  ...
  for color, highlight_group in pairs(colors) do
    ...
    if found then
      local group = setmetatable({ ctermfg = fg.cterm, ctermbg = bg.cterm, fg = fg.gui, bg = bg.gui }, {
        __tostring = function()
          return group_name
        end,
      })
...
M.set_active = function(self)
  ...
  return table.concat {
    ...
    self:get_file_state(),
    self:get_git_status(palette.Hint, palette.Warn, palette.Error, palette.Error),
    "%=", -- left / right separator
    "%<", -- Collapse point for smaller screen sizes
    self:highlight(palette.Error),
    format_diagnostics(" :%s", severity.ERROR),
    self:highlight(palette.Warn),
    format_diagnostics(" :%s", severity.WARN),
    self:highlight(palette.Info),
    format_diagnostics(" :%s", severity.INFO),
    self:highlight(palette.Hint),
    format_diagnostics(" :%s", severity.HINT),
