...
function git_diff()
  ...
  if added > 0 then
    result = result
      .. string.format('%%#StatuslineGitAdded#%s %d ', icons.git.Added, added)
  end
  if changed > 0 then
    result = result
      .. string.format(
        '%%#StatuslineGitChanged#%s %d ',
        icons.git.Modified,
        changed
      )
  end
...
function git_component()
  ...
  return string.format(
    '%%#TelescopeResultsDiffDelete# %%#StatuslineItalic#%s %s',
    head,
    git_diff()
  )
end
  ...
  for severity, count in pairs(counts) do
    if count ~= 0 then
      ...
      local part = string.format(
        '%%#%s#%s %%#StatuslineItalic#%d',
        get_or_create_hl(hl),
        icons.diagnostics[icon[severity]],
        count
      )
...
function filetype_component()
  ...
  return string.format(
    '%%#%s#%s %%#StatuslineTitle#%%#StatuslineItalic#%s',
    icon_hl,
    icon,
    filetype
  )
...
function encoding_component()
  ...
  return encoding ~= ''
      and string.format(
        '%%#StatuslineModeSeparatorOther#%%#NvimTreeRootFolder# %%#StatuslineItalic#%s',
        encoding
      )
    or ''
end
